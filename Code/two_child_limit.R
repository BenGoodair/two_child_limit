...
pacman::p_load(devtools, dplyr,pdftools, tidyverse,rattle,glmnet,caret, rpart.plot, RcolorBrewer,rpart, tidyr, mice, stringr,randomForest,  curl, plm, readxl, zoo, stringr, patchwork,  sf, clubSandwich, modelsummary, sjPlot)
...
final_data <- bind_rows(cleaned_data, y2021, y2022, y2023, y2024)%>%
  ...
  dplyr::mutate(Local.authority = Location %>%
                  gsub('&', 'and', .) %>%
                  gsub('[[:punct:] ]+', ' ', .) %>%
                  gsub('[0-9]', '', .)%>%
                  toupper() %>%
...
clustered_se <- coeftest(did_model, vcov = vcovHC(did_model, type = "HC1", cluster = "Local.authority"))
print(clustered_se)
...
aye <- figure %>%  dplyr::mutate(two_child_impact_quantiles =cut(Child.Tax.Credit, 
                                        breaks = quantiles, 
                                        labels = c(1,2,3),
                                        include.lowest = T) )%>%
